...
// }
//Recursive approach
Node* Recursive_Reverse_LL(Node* cur,Node* prev,Node* forward)
{
    if(cur == NULL)
    {
        return prev;
    ...
    forward = cur->next;
    cur->next = prev;
    return Recursive_Reverse_LL(forward,cur,forward);
}
int main()
{
    Node* Head = NULL;
    ...
    Node* prev = NULL;
    Node* forward = NULL;
    Node* rHead = Recursive_Reverse_LL(Head,prev,forward);
    Travel(rHead);
    return 0;
}